...
def compute_scores():
    ...
    for section in SECTIONS:
        ...
        for q in section["questions"]:
            ...
            if max_s == 0:
                continue
            normalized.append(raw / max_s)
        result[section["key"]] = round(sum(normalized) / len(normalized) * 100) if normalized else 0
    return result
...
1. 절대 뭉뚱그리지 말고, 응답 항목을 직접 근거로 들어 분석하라. (예: "귀사는 '{company_name}'의 경우 도입기간이 1~2년임에도
   전담조직이 없다는 점에서...")
2. 8개 영역 각각에 대해 점수, 현황요약, 강점, 약점/리스크, 구체적 실행 권고안(담당 주체 포함)을 제시하라.
...
valid_codes = None
try:
    raw_codes = st.secrets.get("ACCESS_CODES", "")
...
if valid_codes and not st.session_state.unlocked:
    st.info("본 진단은 결제 확인 후 발급된 접근 코드가 필요합니다.")
    code_input = st.text_input("접근 코드 입력", type="password")
    if st.button("코드 확인"):
